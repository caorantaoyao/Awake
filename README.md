...
- DeerFlow 2.0 通过 git submodule 自动包含（clone 时加 `--recurse-submodules`）
...
确保 clone 时包含 submodule（deer-flow 作为 git submodule 管理）：
...
7. **deer-flow/ 是 git submodule**：通过 `git submodule update --init` 拉取，Awaken 追踪 DeerFlow 的特定 commit 以保证兼容性；本地配置（.env、config.yaml）被 .gitignore 排除，不提交到 Awaken 仓库。
